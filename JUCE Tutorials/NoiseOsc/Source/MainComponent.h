		...
		if (samplesToTarget > 0)
		{
			...
			for (auto sample = 0; sample < numSamplesThisTime; ++sample)
			{
				for (auto channel = 0; channel < bufferToFill.buffer->getNumChannels(); ++channel) {
					//int temp_rand = random.nextFloat() > 0.5f ? 1 : 0;
					//bufferToFill.buffer->setSample(channel, sample, temp_rand * currentLevel);
		...
		if (numSamplesRemaining > 0)
		{
			for (auto channel = 0; channel < bufferToFill.buffer->getNumChannels(); ++channel)
			{
				...
				for (auto sample = 0; sample < numSamplesRemaining; ++sample) {
					//int temp_rand = random.nextFloat() > 0.5 ? 1 : 0;
					//buffer[sample] = temp_rand * currentLevel;
					buffer[sample] = random.nextFloat() * currentLevel;
				}
			}
		}
